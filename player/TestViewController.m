...
- (void)updateOutlook
{
	switch (self.infoLabelStatus) {
		...
		case InfoStatusNormal:
			self.infoLabel.text = @"画图吧";
           // ViewController *controller=[[ViewController alloc] init];
			break;
		case InfoStatusFailedMatch:
			self.infoLabel.text = [NSString stringWithFormat:@"画错了,再来 !",self.wrongGuessCount];
			break;
...
- (void)lockScreen:(SPLockScreen *)lockScreen didEndWithPattern:(NSNumber *)patternNumber
{
	...
	switch (self.infoLabelStatus) {
		case InfoStatusFirstTimeSetting:
			[stdDefault setValue:patternNumber forKey:@"KeyForCurrentPatternToUnlockTemp"];
			self.infoLabelStatus = InfoStatusConfirmSetting;
			[self updateOutlook];
			break;
		case InfoStatusFailedConfirm:
			[stdDefault setValue:patternNumber forKey:@"KeyForCurrentPatternToUnlock"];
			self.infoLabelStatus = InfoStatusConfirmSetting;
			[self updateOutlook];
			break;
		case InfoStatusConfirmSetting:
			if([patternNumber isEqualToNumber:[stdDefault valueForKey:@"KeyForCurrentPatternToUnlockTemp"]]) {
				[stdDefault setValue:patternNumber forKey:@"KeyForCurrentPatternToUnlock"];
			...
			break;
		case  InfoStatusNormal:
			if([patternNumber isEqualToNumber:[stdDefault valueForKey:@"KeyForCurrentPatternToUnlock"]]) [self dismissViewControllerAnimated:YES completion:nil];
			else {
